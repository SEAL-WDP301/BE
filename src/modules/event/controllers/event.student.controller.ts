...
export class EventStudentController {
  ...
  async getEventDetail(
    ...
    const detail = await this.eventStudentService.getEventDetail(eventId, Number(userId));
  ...
  async registerIndividual(
    ...
    const registration = await this.eventStudentService.registerIndividual(Number(userId), eventId, dto);
    return { message: 'Individual registration successful', data: registration };
  }
  ...
  async registerTeam(
    ...
    const team = await this.eventStudentService.registerTeam(Number(userId), eventId, dto);
  ...
  async getInvitations(@CurrentUser('id') userId: string) {
    const invitations = await this.eventStudentService.getInvitations(Number(userId));
  ...
  async acceptInvitation(
    ...
    const updated = await this.eventStudentService.respondToInvitation(Number(userId), teamId, true);
  ...
  async rejectInvitation(
    ...
    const updated = await this.eventStudentService.respondToInvitation(Number(userId), teamId, false);

...
export class EventOrganizerService {
  ...
  async updateTeamStatus(teamId: number, dto: OrganizerUpdateTeamDto, adminId: number) {
    ...
    const updated = await this.prisma.team.update({
      ...
      data: {
        status: dto.status,
        eliminationReason: dto.status === TeamStatus.eliminated ? dto.reason : null,
      },
    });

    // Send notifications
    const emailsToNotify = [team.leader.email, ...team.members.map(m => m.user.email)];
    
    // Create notifications in DB
    const notifications = emailsToNotify.map(email => {
      const user = email === team.leader.email ? team.leader : team.members.find(m => m.user.email === email)?.user;
      if (user) {
        return {
          userId: user.id,
    ...
    emailsToNotify.forEach(email => {
      this.logger.log(`[MOCK MAIL] Sending email to ${email} regarding team ${team.name} status change to ${dto.status}`);
